...
runCell <- function(rp, compData, missData, valData, impLists, parms) {
  ...
  compFit <- fitModels(compData, parms) ### KML
  compOut <- getStats(compFit, valData, parms) ### KML
  ...
  ### BVV: changed to separate val set for everything to evaluate on
...
doRep <- function(rp, conds, parms) {
  ...
  for(i in 1 : nrow(conds)) {
    ...
    ### BVV : Added separate validation set to this if statement.
    check <-
    ...
    ### BVV: removed this and added a separate validation set to the if statement above.
